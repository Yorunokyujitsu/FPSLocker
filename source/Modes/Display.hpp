...
public:
	uint64_t tick;
    size_t i;
	char Docked_c[512] ="이 메뉴는 60 Hz 미만에서 지원되는\n"
	...
	virtual bool handleInput(u64 keysDown, u64 keysHeld, const HidTouchState &touchPos, HidAnalogStickState joyStickPosLeft, HidAnalogStickState joyStickPosRight) override {
		...
		if (R_SUCCEEDED(ommGetDefaultDisplayResolution(&width, &height))) {
			if (height != 720 && height != 1080) {
				snprintf(PressButton, sizeof(PressButton), "%dp를 지원하지 않습니다!", (uint16_t)height);
				return true;
			}
		}
		if (check && PluginRunning && (Shared -> pluginActive)) {
				snprintf(PressButton, sizeof(PressButton), "게임을 먼저 종료하세요!");
				return true;			
		}
...
public:
    ...
    DockedOverWizardGui(uint8_t maxRefreshRate) {
		...
		snprintf(Docked_c, sizeof(Docked_c), "이 메뉴는 60Hz를 넘는 지원 주사율을\n"
											"최대 %d Hz까지 차례로 테스트합니다.\n"
											"1080p에서만 필요하며, 720p는\n"
											"별도 설정 없이 작동합니다.\n\n"
											"각 주사율이 정상 작동하는지 확인하려면\n"
											"표시된 버튼을 입력하세요.\n\n"
											"10초 이내 입력이 없으면 스킵됩니다.\n"
											"최대 %d초가 소요될 수 있습니다.", maxRefreshRate, delay_s * times);
	}

    virtual tsl::elm::Element* createUI() override {
        auto frame = new tsl::elm::OverlayFrame("FPSLocker", "디스플레이 오버클럭 마법사");
...
public:
    DockedGui() {
		...
		if (R_SUCCEEDED(setsysGetEdid(&edid))) {
			...
			snprintf(Docked_c, sizeof(Docked_c), "확인된 최대 주사율: %.2f Hz\n설정 ID: %08X", highestRefreshRate, crc32);
		}
		setsysExit();
		smExit();
	}
    ...
    virtual tsl::elm::Element* createUI() override {
		...
		auto *clickableListItem1 = new tsl::elm::ListItem("주사율 허용 범위");
		...
		auto *clickableListItem2 = new tsl::elm::ListItem("주사율 마법사");
		...
		if (highestRefreshRate >= 70.f) {
			auto *clickableListItem22 = new tsl::elm::ListItem("1080p 오버클럭 마법사");
		...
		auto *clickableListItem3 = new tsl::elm::ListItem("프레임 스킵 테스터");
		...
		auto *clickableListItem4 = new tsl::elm::ListItem("추가 설정");
		clickableListItem4->setClickListener([this](u64 keys) { 
			if ((keys & HidNpadButton_A) && !block) {
    ...
    virtual tsl::elm::Element* createUI() override {
		...
		if (!displaySync) {
			if (entry_mode == ApmPerformanceMode_Normal) {
				auto *clickableListItem = new tsl::elm::ListItem("주사율 ");
				...
				auto *clickableListItem2 = new tsl::elm::ListItem("주사율 ");
				clickableListItem2->setClickListener([this](u64 keys) { 
					if ((keys & HidNpadButton_A) && (!isOLED || isDocked)) {
						if (refreshRate_g > 40) {
							if (R_SUCCEEDED(SaltySD_Connect())) {
				...
				auto *clickableListItem2 = new tsl::elm::ListItem("주사율 변경");
		...
		if (!oldSalty) {
			...
			if (!isLite) {
				auto *clickableListItem4 = new tsl::elm::ListItem("독 모드 설정");
				clickableListItem4->setClickListener([this](u64 keys) { 
					if ((keys & HidNpadButton_A)) {
...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
		...
		auto *clickableListItem2 = new tsl::elm::ListItem("동의");
		clickableListItem2->setClickListener([this](u64 keys) { 
			if ((keys & HidNpadButton_A)) {
				tsl::goBack();
				tsl::changeTo<DisplayGui>();
