...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
		auto frame = new tsl::elm::OverlayFrame("버퍼링 설정", " ");
...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
		...
		list->addItem(new tsl::elm::CategoryHeader("기타", false));

		auto *clickableListItem5 = new tsl::elm::MiniToggleListItem("게임 창이 포커스를 잃으면 중단", forceSuspend_save);
		...
		if (PluginRunning) {
			if (i > 9) {
				...
				switch (Shared -> API) {
					...
					case 3: {
						if (((Shared -> Buffers) >= 2 && (Shared -> Buffers) <= 4)) {
							sprintf(&nvnBuffers[0], "활성화된 버퍼: %d", (Shared -> Buffers));
						}
						break;
					}
				}
